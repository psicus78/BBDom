...
namespace KNXLib.DPT
{
    internal sealed class DataPoint8BitSignRelativeValue : DataPoint
    {
        ...
        {
            get { return new[] { "6.001", "6.010", "20.102" }; }
        }
        ...
        public override byte[] ToDataPoint(object val)
        {
            ...
            {
                _logger.Error("6.xxx", "input value received is not a valid type");
                return dataPoint;
            }
            
            if (input < -128 || input > 127)
            {
                _logger.Error("6.xxx", "input value received is not in a valid range");
                return dataPoint;
            }
